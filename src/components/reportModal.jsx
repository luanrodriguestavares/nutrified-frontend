...
// Configuração do modal
Modal.setAppElement('#root');

export function ReportModal({ isOpen, onRequestClose, title, value, goal, remaining, unit, isOverGoal }) {
    ...
    return (
        ...
        >
            <button onClick={handleClose} className="absolute top-4 right-4 text-gray-500 hover:text-gray-700 focus:outline-none z-40">
                <X className="w-6 h-6" />
            </button>
                ...
                {isOverGoal && (
                    <div className="bg-rose-400 p-4 rounded-xl text-white transform transition-transform duration-200">
                        <h3 className="font-semibold">
                            Aviso <TriangleAlert className="inline" />
                        </h3>
                        <p className="text-gray-50">
                            Você ultrapassou sua meta diária em: {exceededAmount} {unit}
                        </p>
                    </div>
                )}
            </div>
